    ...
    module Firebird
      ...
      class SchemaCreation < ActiveRecord::ConnectionAdapters::SchemaCreation
        ...
        def visit_ColumnDefinition(o)
          ...
          elsif o.type == :bigint && o.primary_key?
            "#{quote_column_name(o.name)} BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY"
          else
            super
          end
        end
        ...
        # CRITICAL: Firebird requires DEFAULT before NOT NULL
        def add_column_options!(sql, options)
          column = options.fetch(:column) { return super }
          return sql if (column.respond_to?(:virtual?) && column.virtual?) || (column.type == :primary_key)
          return sql if column.type == :bigint && column.primary_key?

          if options.key?(:default) && !options[:auto_increment]
            default = options[:default]
            sql << " DEFAULT #{quote_default_expression(default, column)}"
          elsif %i[datetime timestamp].include?(column.type)
