    ...
    module Firebird
      module SchemaStatements
        ...
        def table_exists?(table_name)
          table_name = quote(table_name.to_s.upcase)
        ...
        def view_exists?(view_name)
          query_value(<<~SQL, "SCHEMA") == 1
            SELECT COUNT(*)
            FROM RDB$RELATIONS
            WHERE UPPER(TRIM(RDB$RELATION_NAME)) = UPPER(#{quote(view_name.to_s.upcase)})
            AND RDB$VIEW_BLR IS NOT NULL
          SQL
        end
        ...
        def indexes(table_name)
          result = query(<<~SQL, "SCHEMA")
            ...
            WHERE UPPER(TRIM(i.RDB$RELATION_NAME)) = UPPER(#{quote(table_name.to_s.upcase)})
        ...
        def column_definitions(table_name)
          ...
          sql = <<~SQL
            ...
            WHERE r.rdb$relation_name = #{quote(table_name.to_s.upcase)}
          ...
          result.map do |row|
            ...
            identity_type = scalar_value(row[11])
            ...
            sql_type = case field_type
                       ...
                       when 16
                         ...
                         elsif field_sub_type == 2 || (field_sub_type == 1 && field_scale && field_scale < 0)
    ...
    type_metadata = fetch_type_metadata(sql_type, cast_type)
        ...
        def primary_keys(table_name)
          keys = query_values(<<~SQL, "SCHEMA")
            ...
            WHERE rc.RDB$RELATION_NAME = #{quote(table_name.to_s.upcase)}
        ...
        def foreign_keys(table_name)
          fk_info = query(<<~SQL, "SCHEMA")
            ...
            WHERE UPPER(TRIM(rc.RDB$RELATION_NAME)) = UPPER(#{quote(table_name.to_s.upcase)})
        ...
        def rename_table(table_name, new_name)
          ...
          raise StatementInvalid, "Table #{table_name} does not exist" unless table_exists?(table_name)
        ...
        def sequence_exists?(sequence_name)
          query_value(<<~SQL, "SCHEMA") == 1
            SELECT COUNT(*)
            FROM RDB$GENERATORS
            WHERE UPPER(TRIM(RDB$GENERATOR_NAME)) = UPPER(#{quote(sequence_name.to_s.upcase)})
            AND RDB$SYSTEM_FLAG = 0
          SQL
        end
